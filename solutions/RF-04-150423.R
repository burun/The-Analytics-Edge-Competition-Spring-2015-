...
blogRF_1 = randomForest(Popular~.,data=dtmTrain,type="prob",ntree=1000,nodesize=5)
blogRF_2 = randomForest(Popular~.,data=dtmTrain,type="prob",ntree=1200,nodesize=4)


PredTest_1 = predict(blogRF_1, newdata=dtmTest, type="prob")[,2]
PredTest_2 = predict(blogRF_2, newdata=dtmTest, type="prob")[,2]

library(ROCR)
PredTrain_1 = predict(blogRF_1, data=dtmTrain, type="prob")[,2]
PredTrain_2 = predict(blogRF_2, data=dtmTrain, type="prob")[,2]
predROCR = prediction((PredTrain_1+PredTrain_2)/2, dtmTrain$Popular)
...
MySubmission = data.frame(UniqueID = NewsTest$UniqueID, Probability1 = (PredTest_1+PredTest_2)/2)

write.csv(MySubmission, "SubmissionRF_09.csv", row.names=FALSE)
